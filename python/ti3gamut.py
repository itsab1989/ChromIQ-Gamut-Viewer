...
window.cqSpin = (function () {
  ...
  return {set: set, nudge: nudge, reset: reset, zoom: zoom, slide: slide,
          ...
          touchedYet: function (id) { return !!touched[id]; }};
})();
"""
...
<title>{_t} — ChromIQ Gamut Viewer</title><style>
 ...
 /* TWO ROOMS OF HALF A PHONE ARE TWO ROOMS OF NOTHING, and the shape does
    not stay inside one. Measured on this page, spin paused and both rooms
    pinned to the same four cameras, counting the vividly coloured pixels
    (the shape; type and gridlines are grey) in each room's outermost column:
    ...
    A single-room page at the same widths is clean, so this is the split and
    not the scene: a 3D box is fitted to the room's HEIGHT, and half a narrow
    window leaves the shape nowhere to go but through the wall. The modebar
    was already turned off at 1024 and the titles shrunk at 820 for the same
    narrowness -- what was never done was to stop halving the width. Below
    that the rooms stack, each gets the whole width, and the page scrolls to
    the second one the way it already scrolls to the controls. */
 @media (max-width:1000px) {{
   .row {{ flex-direction:column; max-height:none; }}
   /* 68vh, not half the screen: stacked, the first room IS the first screen,
      and `check_layout` asks that a picture hold 55-85% of it -- the rule
      that caught a WebKit scene collapsing to a third of an iPad. At 56vh
      each room measured 51-54% and failed it at four sizes in both engines,
      which was this layout being honest about a picture too small to read. */
   .half {{ min-height:68vh; }}
   .half + .half {{ border-left:none;
                    border-top:1px solid {colours['grid']}; }}
 }}
