...
window.cqSpinControls = function (settings) {
  ...
  function button(what, label, title) {
    return '<button type="button" data-cq="' + what + '" title="' + title
           + '">' + label + '</button>';
  }
  ...
  if (HAS_PANEL)
    head += button("more", "more…", "Everything else you can change about "
      ...
      + "its lettering are drawn, whether the names are shown, and whether "
      + "the page is light or dark.");
  bar.innerHTML = head;

  var body = "";
  if (on("lr", true))
    body += '<div class="cq-row"><span>left &amp; right</span>'
      + '<span class="cq-ctl">'
      ...
      + (on("speed_each", false)
         ? button("turn-slower", "&minus;", "Turn left and right more slowly.")
           + '<span data-cq="turn-speed">' + Math.round(speeds.turn) + '</span>'
           + button("turn-faster", "+", "Turn left and right more quickly.")
         : "")
      + '</span></div>';
  if (on("ud", true))
    body += '<div class="cq-row"><span>up &amp; down</span>'
      + '<span class="cq-ctl">'
      ...
      + (on("speed_each", false)
         ? button("tilt-slower", "&minus;", "Tip more slowly.")
           + '<span data-cq="tilt-speed">' + Math.round(speeds.tilt) + '</span>'
  ...
  if (on("appearance", false))
    body += '<div class="cq-row"><span>light or dark</span>'
      + '<span class="cq-ctl">'
  ...
  function paint() {
    css.textContent =
      ...
      + "gap:8px 22px;max-width:900px;margin:0 auto;width:100%;"
  ...
  function press(what, state) {
    var el = bar.querySelector('[data-cq=' + what + ']')
