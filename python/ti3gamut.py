...
(function () {
  function fit(gl) {
    try {
      ...
      // box BEFORE the model matrix; the camera lives after it. Read off a
      // running page: bounds spanned 1.0625 on every axis while the model
      // scaled by (1.0310, 1.3829, 0.5847) and shifted z by -0.2924, so the
      // drawn box was off-centre by half a unit in z and up to 38% the wrong
      // size per axis. Fitted to that, the far plane cut the axis box away at
      // the cameras four buttons on every saved page produce: 113,649 pixels
      // erased at "above", 79,031 at "angle", 256,077 looking along b*, with
      // the axis titles printed twice.
      //
      // The drawn box is exactly +/- aspect/2 about the origin -- checked
      ...
      // ⚠ AND THE PAD HAS TO COVER WHAT LIVES OUTSIDE THE BOX. The shapes sit
      // inside it; the tick marks, the tick numbers and the axis titles do
      // not, and a pad sized only for rounding clips them. Measured by the
      // pixels the library draws that this erases, at the page's own
      // "look from" buttons -- see the test.
      // ⚠ A SHARE OF THE BOX, NOT OF THE VIEW-ALIGNED SPAN. The tick marks,
      // the tick numbers and the axis titles are drawn OUTSIDE the box, and
      // how far outside does not change with the camera -- but `hi - lo`
      // does: looking straight down an axis the box is barely deep at all, so
      // a share of it collapses exactly where the furniture needs covering.
      // Measured, pixels the library drew that this erased, looking along b*:
      // a fifth of the span erased 13,419; a fifth of the box, 31.
      // And widening the SPAN share until the clipping stopped brought the
      // hatching back -- at 1.0 the lid's own speckle went 3 -> 207 -- because
      // the pad then swallows the precision at every other camera too.
      var across = Math.sqrt(a[0] * a[0] + a[1] * a[1] + a[2] * a[2]);
      // HALF THE BOX ACROSS, measured at both ends. Pixels the library drew
      // that this erases, and the speckle the lid adds, on his own pair:
      //
      //     pad      along b*   along a*   above  |  hatch at four cameras
      //     0.2      13,413     10,889    8,156   |   3    0    3    4
      //     0.5          28         53       28   |   2    5   17  132
      //     0.9           -          -        -   |  227  112  230  130
      //
      // A fifth of the box is tight enough to leave the axis furniture
      // outside the far plane; nine tenths spends so much depth on empty
      // space that the hatching comes back. Half clears the furniture at
      // every camera and keeps the hatching cured at three of the four --
  ...
  function arm(gd) {
    ...
    Object.keys(gd._fullLayout).forEach(function (key) {
      ...
      if (!gl) return;
      // ⚠ THE MARK GOES ON THE FUNCTION, NOT ON THE SCENE. Written as a flag
      // on the object it says "armed" for ever -- so if the library ever
      // assigns `onrender` again, this one is gone and is never put back, and
      // the only symptom is the hatching quietly returning.
      if (gl.onrender && gl.onrender.__cqDepth) return;
  ...
  function sweep() {
    ...
    for (n = 0; n < all.length; n++) {
      gd = all[n];
      arm(gd);
      // ⚠ AND ARM WHATEVER IS DRAWN NEXT, FOR AS LONG AS THE PAGE LIVES. A
      // scene is rebuilt whenever the reader opens another file, changes the
